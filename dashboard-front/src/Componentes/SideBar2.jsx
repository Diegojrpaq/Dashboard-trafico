...
export default function SideBar2() {
  ...
  let permisos = [];
  if (sessionUserState.User.permisos != null) {
    // Si el usuario tiene permisos que se agreguen al arreglo permisos
  ...
  return (
    <>
      <div
        className={
          toggleSidebar ? "sidebar-hidden" : "sidebar d-none d-md-block"
        }
      >
        ...
        <SHeaderList title="Trafico" icon="bi bi-globe-americas" idcollapse="1">
          {permisos.includes(31) ? (
            <SListItem
              icon="bi bi-clipboard2-data mx-1"
              url={"/trafico/ResumenPlaneacion/"}
            >
              Resumen Planeación Rutas
            </SListItem>
          ) : (
            <></>
          )}
          ...
          {permisos?.includes(31) ? (
            ...
            >
              {destinosConRutas &&
                destinosConRutas.map((destino, index) => {
                  return (
                    destino.sucursales && (
                      <SHeaderList
                        title={destino.nombre}
                        icon="bi bi-geo-alt mx-1"
                        key={destino.id + 100 + index}
                        idcollapse={destino.id + 100}
                      >
                          ...
                          if (sucursal.rutas_configuradas) {
                            return (
                              <SHeaderList
                                ...
                                icon="bi bi-shop-window mx-1"
                                key={sucursal.id_sucursal + 200}
                                idcollapse={sucursal.id_sucursal + 200}
                              >
                                {sucursal?.rutas_configuradas?.map((ruta) => {
                                  return (
                                    <SListItem
                                      icon="bi bi-bus-front mx-3"
                                      key={ruta.id_ruta + "suc"}
                                      url={
                                        "/trafico/planeacion/" +
          ...
          {permisos.includes(32) ? (
            <SHeaderList
              title="Plan Llegada Destinos Por Recibir"
              icon="bi bi-clipboard2-check-fill"
              key={5}
              idcollapse="5"
            >
              {destConfigLlegadas &&
                destConfigLlegadas.map((destino) => {
                  return (
                    <SListItem
                      icon="bi bi-geo-alt mx-1"
                      key={destino.id + destino.nombre}
                      url={"/trafico/planeacionLlegadas/" + destino.id}
                    >
                      {destino.nombre}
                    </SListItem>
                  );
...

          {
            ...
            permisos.includes(32) ? (
              <SHeaderList
                title="Plan Llegada Destinos Para Enviar"
                icon="bi bi-clipboard2-check-fill"
                key={6}
                idcollapse="6"
              >
                  ...
                  destConfigLlegadas.map((destino) => {
                    return (
                      <SListItem
                        icon="bi bi-geo-alt mx-1"
                        key={destino.id + destino.nombre}
                        url={"/trafico/planLlegadaGrafo/" + destino.id}
                      >
                        {destino.nombre}
                      </SListItem>
                    );
          ...
          {permisos.includes(32) ? (
            <SListItem
              ...
              url={"/trafico/planeacionDomicilio"}
            >
              Planeación Domicilio
            </SListItem>
          ) : (
            <></>
          )}
                ...
                destinosList.map((destino) => {
                  return (
                    <SListItem
                      icon="bi bi-geo-alt mx-1"
                      key={destino.id + destino.nombre}
                      url={"/trafico/viajesactivos/" + destino.id}
                    >
                      {destino.nombre}
                    </SListItem>
                  );
          ...
          {permisos.includes(33) ? (
            <SHeaderList
              title="Viajes por Llegar"
              icon="bi bi-airplane-fill"
              key={4}
              idcollapse="4"
            >
              {destinosXllegar &&
                destinosXllegar.map((destino) => {
                  return (
                    <SListItem
                      icon="bi bi-geo-alt mx-1"
                      key={destino.id}
                      url={"/trafico/viajesxllegar/" + destino.id}
                    >
                      {destino.nombre}
                    </SListItem>
                  );
          ...
          {permisos.includes(34) ? (
            <SListItem
              icon="bi bi-geo-alt mx-2"
              url={"/trafico/viajesHistorico"}
            >
              Histórico
            </SListItem>
          ) : (
          ...
          {permisos.includes(36) ? (
            <SListItem
              icon="bi bi-file-earmark-spreadsheet mx-2"
              url={"/trafico/reporteViajes"}
            >
              Reporte Viajes Histórico
            </SListItem>
          ) : (
            <></>
          )}
