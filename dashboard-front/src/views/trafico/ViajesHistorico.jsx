...
export default function ViajesHistorico() {
  ...
  const peticionInfoField = async () => {
    const fechaAConsultar =
      date && date.toISOString().split("T")[0].replace(/-/g, "");
    const urlApiNextpack =
      urlapi + "/trafico/get_infoHistorico/" + fechaAConsultar;
    await fetch(urlApiNextpack)
      .then((resp) => {
        return resp.json();
      })
  ...
  const peticionViaje = async () => {
    const urlApiNextpack =
      urlapi + "/trafico/get_viajeHistorico/" + selectedViaje.id_viaje;
    await fetch(urlApiNextpack)
      .then((resp) => {
        return resp.json();
      })
      .then((data) => {
        if (data) {
          ...
          const viajeError = bitacoraVSembarcadas(
            generarParadas(data.viaje.Bitacora),
            data.viaje.catalogoGuias
          );
          if (viajeError.error) {
            Swal.fire({
              icon: "error",
  ...
  useEffect(() => {
    if (selectedDestino !== null) {
      const listaTemporal = infoGlobal.filter(
        (destino) => destino.nombre_destino === selectedDestino.nombre_destino
      );
      ...
      <>
        ...
        <div className="col-sm-12 col-md-6 col-lg-4 py-3 px-3">
          <div className="card flex shadow justify-content-center">
            <Dropdown
              disabled={destinosList === null}
              value={selectedDestino}
              onChange={(e) => setselectedDestino(e.value)}
              options={destinosList}
              optionLabel="nombre_destino"
        ...
        <div className="col-sm-12 col-md-12 col-lg-4 py-3 px-3">
          <div className="card flex shadow justify-content-center">
            <Dropdown
              disabled={viajesList === null}
              value={selectedViaje}
              onChange={(e) => setselectedViaje(e.value)}
              options={viajesList}
              optionLabel="nombre_viaje"
